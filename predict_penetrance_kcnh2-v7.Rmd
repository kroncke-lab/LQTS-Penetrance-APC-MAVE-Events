...
modify_split_records <- function(record) {
  ...
  record$time_sex <- ifelse(record$Age_at_event_end < 12, 0, 1)

  # Check if a new record needs to be added
  if (record$Age_at_event_start < 12 && record$Age_at_event_end > 12) {
    # Create a new record with adjusted values
    new_record <- record
    new_record$Age_at_event_start <- 12
    new_record$time_sex <- 1

    # Modify the original record
    record$Age_at_event_end <- 12
    record$time_sex <- 0
    record$Event <- 0
...
covariates <- c("revel_score", "lqt2_dist", "traff_score")#, "peak_tail"), "traff_score" ,"am_pathogenicity"
...
covariates <- c("revel_score", "lqt2_dist") 
...
tmp_selected <- tmp[tmp$var!="K897T", c("var", "blast_pssm", "provean_score", "pamscore", "aasimilaritymat", "revel_score", "cardiacboost", "am_pathogenicity", "lqt2", "unaff", "resnum", "wtAA", "mutAA", "clinvar.description", "gnomad.v4", "clinvar", "peak_tail", "traff_score", "score_SE", "cohort", "total_carriers", "penetrance_lqt2", "weight", "lqt2_dist", "lqt2_dist_weight", "lqt2_penetranceBayesian_initial", "lqt2_penetranceBayesian", "p_mean_w", "prior_mean")]
...
first.event[!is.na(first.event$QTc) & first.event$QTc > 800,"QTc"] <- 800
first.event$QTc.adj <- first.event$QTc/10

first.serious.event<-merge(first_serious_event_data.cohort, tmp_selected, by = "var", all.x = TRUE)
first.serious.event[!is.na(first.serious.event$QTc) & first.serious.event$QTc > 800,"QTc"] <- 800
first.serious.event$QTc.adj <- first.serious.event$QTc/10

cohort.var <- merge(cohort, tmp_selected, by = "var", all.x = TRUE)
cohort.var[!is.na(cohort.var$QTc) & cohort.var$QTc > 800,"QTc"] <- 800
cohort.var$QTc.adj <- cohort.var$QTc/10
...
linear_model <- lm(Event ~ Sex + QTc.adj.s + p_mean_w + clinvar + revel_score + peak_tail + traff_score + prior_mean + lqt2_dist + am_pathogenicity, data = t) # + prior_mean + lqt2_dist
vif_values <- vif(linear_model)
print(vif_values)
# Remove lqt2_dist
linear_model <- lm(Event ~ Sex + QTc.adj.s + p_mean_w + clinvar + revel_score + peak_tail + traff_score + am_pathogenicity, data = t) # + prior_mean + lqt2_dist
vif_values <- vif(linear_model)
print(vif_values)
# Identifying complete cases for relevant columns
complete_cases <- complete.cases(first.event[, c("p_mean_w", "QTc.adj", "peak_tail", "Sex", "Patient", "clinvar", "revel_score", "lqt2_dist", "am_pathogenicity","traff_score", "observed.lqt2")]) #"traff_score",
...
combined_data <- cbind(first.event.complete[, c("p_mean_w", "peak_tail","traff_score", "revel_score", "lqt2_dist", "am_pathogenicity", "observed.lqt2", "clinvar", "QTc.adj")], dummy_vars) #"traff_score",
...
```{r}
age_cutoff <- 100
t <- cohort.var[!is.na(cohort.var$p_mean_w) & !is.na(cohort.var$QTc.adj) & cohort.var$Age_at_event_end<age_cutoff,]
...
QTc_adj_window_width <- 1   # e.g., from 40 to 50

# Define the range of thresholds to start the sliding windows
p_mean_w_thresholds_start <- seq(0.01, max(t$p_mean_w) - p_mean_w_window_width, by = 0.05)
QTc_adj_thresholds_start <- seq(38, 65 - QTc_adj_window_width, by = 0.5)
...
for (p_start in p_mean_w_thresholds_start) {
  for (q_start in QTc_adj_thresholds_start) {
    ...
    subset_data <- t %>%
      filter(p_mean_w > p_start, p_mean_w <= p_end, 
    ...
    if (nrow(subset_data) > 0) {
      ...
      alpha_value <- ifelse(count <= 2, count / 2, 1)  # Adjust alpha for counts <= 5
...
ggplot(results, aes(x = p_mean_w_threshold_start, y = QTc_adj_threshold_start, fill = event_rate_log, alpha = alpha)) +
  ...
  labs(title = "Event Rates for Sliding Windows of penetrance current and QTc.adj",
...
#t <- first.serious.event[ !is.na(first.serious.event$lqt2_dist) & !is.na(first.serious.event$revel_score) & !is.na(first.serious.event$peak_tail) & !is.na(first.serious.event$p_mean_w) & !is.na(first.serious.event$Sex) & !is.na(first.serious.event$Age_at_event_end) & !is.na(first.serious.event$traff_score),] 
...
x <- model.matrix(~ Sex + QTc.adj + traff_score + peak_tail + p_mean_w + revel_score + am_pathogenicity + clinvar, data = t) #
set.seed(123)  # for reproducibility
cv_fit <- cv.glmnet(x, survival_data, family = "cox", alpha = 0.5)
...
ci_results <- bootstrap_ci(final_model, t, x, n_boot=100)
...
model <- Surv(Age_at_event_start, Age_at_event_end, Event, type = "counting") ~ #
        Sex + QTc.adj + peak_tail + p_mean_w #+ revel_score + am_pathogenicity + clinvar
...
time_point <- 20 # for example, 1 year

# Calculate time-dependent AUC
auc_c_stat <- survivalROC::survivalROC(Stime = t$Age_at_event_end,
                                       status = t$Event,
...
risk_scores <- predict(fit, newdata = cohort.var, type = "risk")
...
cohort.var$QuantileGroup <- cut(risk_scores, breaks = c(-Inf, quantiles, Inf), labels = FALSE, include.lowest = TRUE)

# Assuming your Cox model `fit` is still available
cohort.var$HR <- exp(risk_scores)

# Calculate average HR for each group
avg_hr_by_group <- aggregate(HR ~ QuantileGroup, data = cohort.var, median)


surv_curve <- survfit(Surv(Age_at_event_end, Event) ~ QuantileGroup, data = cohort.var[cohort.var$Patient=="Family Member",])#[cohort.var$Patient=="Family Member",]

p <- ggsurvplot(
  surv_curve, 
  data = cohort.var[cohort.var$Patient=="Family Member",], #[cohort.var$Patient=="Family Member",]
  risk.table = TRUE,
  pval = TRUE,
  conf.int = TRUE,
  legend.labs = levels(first.event$QuantileGroup),
  mark.time = TRUE,
  tables.height = 0.2,
  tables.y.text = FALSE,
...
```{r}
age_cutoff <- 40
t <- first.serious.event[!is.na(first.serious.event$peak_tail) & !is.na(first.serious.event$traff_score),]
t$Age_at_event_end[t$Age_at_event_end > age_cutoff] <- age_cutoff
t$SeriousCardiacEvent[t$Age_at_event_end == age_cutoff] <- 0

model <- Surv(Age_at_event_start, Age_at_event_end, SeriousCardiacEvent) ~ 
                   Sex + QTc.adj + p_mean_w + peak_tail + traff_score + revel_score + am_pathogenicity + clinvar 
...
# Generate predictions
predicted_probs <- predict(fit, newdata = t, type = "survival", times = time_point)
...
roc_result <- survivalROC::survivalROC(Stime = valid_t$Age_at_event_end, 
                                       status = valid_t$SeriousCardiacEvent, 
                                       marker = valid_predicted_probs, 
                                       predict.time = time_point, 
                                       method = "KM")
roc_result$AUC
roc_result$Survival
...
boot_results <- boot(data = t, statistic = boot_function, R = 100)  # R is the number of bootstrap replicates
...
model_terms <- c("gamma0", "gamma1", "Sex", "QTc.adj", "p_mean_w", "peak_tail", "traff_score", "revel_score", "am_pathogenicity", "clinvar")  # Replace with your actual term names
...
# Assign names based on the model terms
model_terms <- c("gamma0", "gamma1", "Sex", "QTc.adj", "peak_tail")  # Replace with your actual term names
names(hr_means) <- model_terms
names(hr_ci_lower) <- model_terms
names(hr_ci_upper) <- model_terms
...
# Plot ROC curves Royston-Parmar model
...
# Fit the model 2 
fit2 <- flexsurvspline(Surv(Age_at_event_start, Age_at_event_end, Event) ~ Sex + QTc.adj + p_mean_w, data = t)
...
valid_t2$valid_predicted_probs2 <- valid_predicted_probs2

# Compute ROC curve using valid data
roc_result2 <- survivalROC::survivalROC(Stime = valid_t2$Age_at_event_end, 
                                        status = valid_t2$Event, 
                                        marker = valid_t2$valid_predicted_probs2, 
                                        predict.time = time_point, 
                                        method = "KM")
...
valid_t3$valid_predicted_probs3 <- valid_predicted_probs3

# Compute ROC curve using valid data
roc_result3 <- survivalROC::survivalROC(Stime = valid_t3$Age_at_event_end, 
                                        status = valid_t3$Event, 
                                        marker = valid_t3$valid_predicted_probs3, 
                                        predict.time = time_point, 
                                        method = "KM")
...
ggplot(all_roc_data, aes(x = Specificity, y = Sensitivity, color = Model)) +
  ...
  annotate("text", x = 0.6, y = 0.1, label = paste("AUC1:", round(roc_result1$AUC, 2)), size = 4, color = "black") +
  annotate("text", x = 0.6, y = 0.15, label = paste("AUC2:", round(roc_result2$AUC, 2)), size = 4, color = "black") +
  annotate("text", x = 0.6, y = 0.2, label = paste("AUC3:", round(roc_result3$AUC, 2)), size = 4, color = "black")
...
model <- Surv(Age_at_event_start, Age_at_event_end, Event) ~ 
                   Sex + QTc.adj + peak_tail + p_mean_w + traff_score
...
# Choosing a specific time point for prediction (e.g., 5 years)
time_point <- 20
...
# Calculate a concordance statistic
concordance <- concordance(model, data = t)
concordance

nom <- nomogram(fit, fun = exp, fun.at=c(0.25,0.5,1,2,3,4,5), funlabel="Hazard Ratio")
...
# Spearman rank order correlation all covariates

```{r}

```

# Spearman rank order correlation variant-specific covariates

```{r}
# Identifying complete cases for relevant columns
complete_cases <- complete.cases(tmp[, c("p_mean_w", "peak_tail", "clinvar", "revel_score", "lqt2_dist", "am_pathogenicity","traff_score")]) #"traff_score",
# Subset tmp with complete cases
tmp.complete <- tmp[complete_cases, ]
# Combine numeric and dummy variables
combined_data <- tmp.complete[, c("p_mean_w", "peak_tail","traff_score", "revel_score", "lqt2_dist", "am_pathogenicity",  "clinvar")] #"traff_score",
cor_matrix <- cor(combined_data, use = "pairwise.complete.obs", method = "spearman")
cor_matrix
...
# Brier Score and Net Benefit

```{r}

preds <- c("p_mean_w", "traff_score", "peak_tail", "revel_score", "lqt2_dist", "treat_all", "clinvar", "observed.lqt2")
resultTableScore<-data.frame()
nb <- data.frame(threshold=seq(0, 0.4, 1/250))
i<-0
t <- first.event[!is.na(first.event$traff_score) & !is.na(first.event$lqt2_dist) & !is.na(first.event$revel_score) & !is.na(first.event$peak_tail),] 
for (p in preds){
  i = i+1
  resultTableScore[i,"predictor"]<-p
  resultTableScore[i,"Brier Score"]<-brierScore(t, p,"Status", "ReverseStatus")
  resultTableScore[i,"Brier Skill Score"]<- 1 - brierScore(t, p,"Status", "ReverseStatus")/brierScore_ref(t,alpha0,beta0,"Status", "ReverseStatus")
  resultTableScore[i,"Alt Brier Skill Score"]<- 1 - brierScore(t, p,"Status", "ReverseStatus")/brierScore_alt_ref(t,"Status", "ReverseStatus")
  resultTableScore[i,"Net Benefit"]<-netbenefit(t, 0.05, p,"Status", c("Status", "ReverseStatus"))
  nb <- merge(nb,netbenefit_fig(t, 0.05, p,"Status", c("Status", "ReverseStatus")))
  print(p)
}

print(resultTableScore)

nb_melted<-melt(nb,id.vars = 'threshold')
p <- ggplot(nb_melted, aes(threshold, value, color=variable), color=factor(variable)) +
  geom_line(aes(group=factor(variable)),linewidth=2) +
  coord_cartesian(ylim = c(0, 0.6)) +  # Limiting y-axis range
