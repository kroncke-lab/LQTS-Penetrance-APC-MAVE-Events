...
#Import new gnomAD
gnomad.v4<-read.csv('/Users/KRONCKE/Dropbox/sat paper/Modelling/gnomAD_v4.1.0_(non-UKB)_simple.csv')
...
# Plot ROC of peak tail, penetrance and trafficking score against clinvar annotations

```{r}
t <- tmp[!is.na(tmp$traff_score) & !is.na(tmp$p_mean_w) & !is.na(tmp$clinvar),] #!is.na(tmp$peak_tail) & 

# Calculate ROC and AUC for t$peak_tail
roc_result_peak_tail <- roc(response = t$clinvar, predictor = t$peak_tail)
auc_peak_tail <- auc(roc_result_peak_tail)
ci_peak_tail <- ci(roc_result_peak_tail)

# Calculate ROC and AUC for t$p_mean_w
roc_result_p_mean_w <- roc(response = t$clinvar, predictor = t$p_mean_w)
auc_p_mean_w <- auc(roc_result_p_mean_w)
ci_p_mean_w <- ci(roc_result_p_mean_w)

# Calculate ROC and AUC for t$traff_score
roc_result_traff_score <- roc(response = t$clinvar, predictor = t$traff_score)
auc_traff_score <- auc(roc_result_traff_score)
ci_traff_score <- ci(roc_result_traff_score)

# Plotting
plot(roc_result_peak_tail, col="#000000", print.auc=FALSE, main="ROC Curves")
plot(roc_result_p_mean_w, col="#FF0000", print.auc=FALSE, add=TRUE)
plot(roc_result_traff_score, col="#0000FF", print.auc=FALSE, add=TRUE)
legend("bottomright", legend=c("Peak Tail", "P Mean W", "Traff Score"), 
       col=c("#000000", "#FF0000", "#0000FF"), lwd=2)

# Annotations for AUC and CI
text_x <- max(par("usr")[1], par("usr")[2]) * 0.6
text_y <- max(par("usr")[3], par("usr")[4]) * 0.2
offset <- max(par("usr")[3], par("usr")[4]) * 0.08

# Adjust these text positions if necessary
text(text_x, text_y, sprintf("Peak Tail AUC: %.3f, 95%% CI: [%.3f, %.3f]", auc_peak_tail, ci_peak_tail[1], ci_peak_tail[3]), col="#000000", cex=0.7)
text(text_x, text_y - offset, sprintf("P Mean W AUC: %.3f, 95%% CI: [%.3f, %.3f]", auc_p_mean_w, ci_p_mean_w[1], ci_p_mean_w[3]), col="#FF0000", cex=0.7)
text(text_x, text_y - offset * 2, sprintf("Traff Score AUC: %.3f, 95%% CI: [%.3f, %.3f]", auc_traff_score, ci_traff_score[1], ci_traff_score[3]), col="#0000FF", cex=0.7)

```

# Set all VUS/conflicting/unannoated to benign

```{r}

tmp[is.na(tmp$clinvar),"clinvar"]<-0
...
```{r}
t <- cohort.var[ !is.na(cohort.var$lqt2_dist) & !is.na(cohort.var$revel_score) & !is.na(cohort.var$peak_tail) & !is.na(cohort.var$p_mean_w) & !is.na(cohort.var$Sex) & !is.na(cohort.var$Age_at_event_end) & !is.na(cohort.var$traff_score) & cohort.var$Country != "France" & cohort.var$Country != "UK",] 
...
```{r}
age_cutoff <- 40
t <- cohort.var[!is.na(cohort.var$am_pathogenicity) & !is.na(cohort.var$QTc.adj) & cohort.var$Age_at_event_end<age_cutoff & cohort.var$Country != "France",]
...
p_mean_w_window_width <- 0.1  # e.g., from 0.1 to 0.3
QTc_adj_window_width <- 0.2   # e.g., from 40 to 50
...
# Plot correlations between QTc and trafficking score, peak tail current, and penetrance; calculate correlations.

```{r}
t <- first.event[!is.na(first.event$peak_tail) & !is.na(first.event$traff_score) & first.event$Country != "France",]# & first.event$Country != "UK",]

# Define a function to calculate Spearman's correlation
spearman_cor <- function(data, indices) {
  d <- data[indices,] # Bootstrap sample
  cor(d[,1], d[,2], method = "spearman")
}

# Calculate Spearman correlation and its 95% CI for each pair
result_peak_tail_QTc <- cor.test(t$peak_tail, t$QTc, method = "spearman")
boot_data <- t[, c("peak_tail", "QTc")]
boot_results <- boot(data=boot_data, statistic=spearman_cor, R=1000) # R is the number of bootstrap replications

# Calculate 95% CI from the bootstrap results
ci_peak_tail_QTc <- boot.ci(boot_results, type="perc")

# Print results using the percentile method
print(paste("Spearman correlation (peak_tail vs QTc):", result_peak_tail_QTc$estimate))
print(paste("95% CI for Spearman correlation (peak_tail vs QTc) using percentile method:", ci_peak_tail_QTc$percent[4], "-", ci_peak_tail_QTc$percent[5]))

# Example calculation (replace this with your actual calculation results)
spearman_cor_value <- result_peak_tail_QTc$estimate
ci_lower <- sprintf("%.2f",ci_peak_tail_QTc$percent[4])
ci_upper <- sprintf("%.2f",ci_peak_tail_QTc$percent[5])

# Create the scatter plot
p <- ggplot(t, aes(x = peak_tail, y = QTc)) +
  geom_point() + # Add points
  geom_smooth(method = "loess") +#, se = FALSE) + # Add a linear regression line without standard error
  labs(title = "Relationship between Peak Tail and QTc",
       x = "Peak Tail",
       y = "QTc") +
  theme_minimal() +
  # Annotate with Spearman correlation and CI
  annotate("text", x = Inf, y = Inf, hjust = 1.1, vjust = 2,
           label = paste("Spearman's rho =", round(spearman_cor_value, 2), 
...
# Calculate Spearman correlation and its 95% CI for each pair
result_p_mean_w_QTc <- cor.test(t$p_mean_w, t$QTc, method = "spearman")
boot_data <- t[, c("p_mean_w", "QTc")]
boot_results <- boot(data=boot_data, statistic=spearman_cor, R=1000) # R is the number of bootstrap replications

# Calculate 95% CI from the bootstrap results
ci_p_mean_w_QTc <- boot.ci(boot_results, type="perc")

# Print results using the percentile method
print(paste("Spearman correlation (Penetrance vs QTc):", result_p_mean_w_QTc$estimate))
print(paste("95% CI for Spearman correlation (Penetrance vs QTc) using percentile method:", ci_p_mean_w_QTc$percent[4], "-", ci_p_mean_w_QTc$percent[5]))

# Example calculation (replace this with your actual calculation results)
spearman_cor_value <- result_p_mean_w_QTc$estimate
ci_lower <- sprintf("%.2f",ci_p_mean_w_QTc$percent[4])
ci_upper <- sprintf("%.2f",ci_p_mean_w_QTc$percent[5])

# Create the scatter plot
p <- ggplot(t, aes(x = p_mean_w, y = QTc)) +
  geom_point() + # Add points
  geom_smooth(method = "loess") +#, se = FALSE) + # Add a linear regression line without standard error
  labs(title = "Relationship between Penetrance and QTc",
       x = "Penetrance",
       y = "QTc") +
  theme_minimal() +
  # Annotate with Spearman correlation and CI
...
# Calculate Spearman correlation and its 95% CI for each pair
result_traff_score_QTc <- cor.test(t$traff_score, t$QTc, method = "spearman")
boot_data <- t[, c("traff_score", "QTc")]
boot_results <- boot(data=boot_data, statistic=spearman_cor, R=1000) # R is the number of bootstrap replications

# Calculate 95% CI from the bootstrap results
ci_traff_score_QTc <- boot.ci(boot_results, type="perc")

# Print results using the percentile method
print(paste("Spearman correlation (Trafficking Score vs QTc):", result_traff_score_QTc$estimate))
print(paste("95% CI for Spearman correlation (Trafficking Score vs QTc) using percentile method:", ci_traff_score_QTc$percent[4], "-", ci_traff_score_QTc$percent[5]))

# Example calculation (replace this with your actual calculation results)
spearman_cor_value <- result_traff_score_QTc$estimate
ci_lower <- sprintf("%.2f",ci_traff_score_QTc$percent[4])
ci_upper <- sprintf("%.2f",ci_traff_score_QTc$percent[5])

# Create the scatter plot
p <- ggplot(t, aes(x = traff_score, y = QTc)) +
  geom_point() + # Add points
  geom_smooth(method = "loess") +#, se = FALSE) + # Add a linear regression line without standard error
  labs(title = "Relationship between Trafficking Score and QTc",
       x = "Trafficking Score",
       y = "QTc") +
  theme_minimal() +
  # Annotate with Spearman correlation and CI
...
# Royston-Parmar model, non-parametric time-to-event analysis, all Events, traff, peak tail, and penetrance

```{r}
age_cutoff <- 40
t <- first.event[!is.na(first.event$peak_tail) & first.event$Country != "France",]# & !is.na(first.event$peak_tail) ,]# & first.event$Country != "UK",]!is.na(first.event$traff_score) & 
t$Age_at_event_end[t$Age_at_event_end > age_cutoff] <- age_cutoff
t<-t[t$Age_at_event_end > t$Age_at_event_start,]
t$Event[t$Age_at_event_end == age_cutoff] <- 0

# QT and Sex
model <- Surv(Age_at_event_end, Event) ~ 
                   Sex + QTc.adj #+ p_mean_w#+ p_mean_w# + traff_score  + + peak_tail clinvar + pore + am_pathogenicity
...
predicted_values <- predict(fit, type="lp")
# Extract the linear predictor as the risk score
risk_scores <- predicted_values$.pred_link

c_index <- rcorr.cens(risk_scores, Surv(t$Age_at_event_end, t$Event))
print(paste(round(1-c_index[1],3), " +/- ", round(c_index[3],3)))

# Trafficking score
model <- Surv(Age_at_event_end, Event) ~ 
                   Sex + QTc.adj + traff_score

fit <- flexsurvspline(model, 
                   data = t)
fit

# Using fit$res.t to create a data frame for the forest plot, excluding gamma parameters
coef_df <- data.frame(
  Term = rownames(fit$res.t)[-c(1,2)],  # Exclude gamma0 and gamma1 if they're not of interest
  HR = exp(fit$res.t[-c(1,2), "est"]),  # Calculate Hazard Ratios by exponentiating estimates
  Lower95CI = exp(fit$res.t[-c(1,2), "L95%"]),  # Lower bound of CI for HR
  Upper95CI = exp(fit$res.t[-c(1,2), "U95%"])   # Upper bound of CI for HR
)
# Adjusting the dataframe for 'SexFemale'
# Assume coef_df is already created as before, without adjusting for 'SexFemale' yet

# Find the row for 'SexMale', invert the sign of its estimate, and its CIs
# You might not need to recalculate HR since we're changing interpretation, but let's ensure everything is consistent
sex_male_row <- which(coef_df$Term == "SexMale")
if(length(sex_male_row) == 1) {  # Ensure that there is exactly one match
  # Invert signs for interpretation as 'SexFemale'
  coef_df$HR[sex_male_row] <- 1 / coef_df$HR[sex_male_row]  # Inverting HR
  # Swap lower and upper CI bounds before inverting for correct order
  old_lower <- coef_df$Lower95CI[sex_male_row]
  coef_df$Lower95CI[sex_male_row] <- 1 / coef_df$Upper95CI[sex_male_row]
  coef_df$Upper95CI[sex_male_row] <- 1 / old_lower
  
  # Rename the term to 'SexFemale'
  coef_df$Term[sex_male_row] <- "SexFemale"
}

ggplot(coef_df, aes(x = Term, y = HR, ymin = Lower95CI, ymax = Upper95CI)) +
  geom_pointrange() +
  coord_flip() +
  geom_hline(yintercept = 1, linetype = "dashed", color = "blue") + # Corrected line addition
  theme_minimal() +
  xlab("Covariates") +
  ylab("Hazard Ratio (95% CI)") +
  ggtitle("Forest Plot of Survival Model Covariates with SexFemale")

#peak tail current
model <- Surv(Age_at_event_start, Age_at_event_end, Event) ~ 
                   Sex + QTc.adj + peak_tail #+p_mean_w + + peak_tail clinvar + pore + am_pathogenicity
...
# Adjusting the order of factor levels to put SexFemale and QTc.adj at the bottom
desired_order <- c(setdiff(coef_df$Term, c("SexFemale", "QTc.adj", "peak_tail")), "QTc.adj","SexFemale","peak_tail")
coef_df$Term <- factor(coef_df$Term, levels = desired_order)

# Now, plot with the adjusted order
ggplot(coef_df, aes(x = Term, y = HR, ymin = Lower95CI, ymax = Upper95CI)) +
  geom_pointrange() +
  coord_flip() +
  ...
  xlab("Covariates") +
  ylab("Hazard Ratio (95% CI)") +
  ggtitle("Forest Plot of Survival Model Covariates")

#Penetrance
model <- Surv(Age_at_event_start, Age_at_event_end, Event) ~ 
                   Sex + QTc.adj + p_mean_w #+p_mean_w + + peak_tail clinvar + pore + am_pathogenicity
...
# Adjusting the order of factor levels to put SexFemale and QTc.adj at the bottom
desired_order <- c(setdiff(coef_df$Term, c("SexFemale", "QTc.adj", "p_mean_w")), "QTc.adj","SexFemale","p_mean_w")
coef_df$Term <- factor(coef_df$Term, levels = desired_order)
...
#Penetrance + peak tail
model <- Surv(Age_at_event_start, Age_at_event_end, Event) ~ 
                   Sex + QTc.adj + p_mean_w + peak_tail #+ traff_score
...
# Adjusting the order of factor levels to put SexFemale and QTc.adj at the bottom
desired_order <- c(setdiff(coef_df$Term, c("SexFemale", "QTc.adj", "p_mean_w","peak_tail","traff_score")), "QTc.adj","SexFemale","p_mean_w","peak_tail","traff_score")
coef_df$Term <- factor(coef_df$Term, levels = desired_order)
...
#Penetrance + peak tail + trafficking
model <- Surv(Age_at_event_start, Age_at_event_end, Event) ~ 
                   Sex + QTc.adj + p_mean_w + peak_tail + traff_score
...
age_cutoff <- 40
t <- first.event[!is.na(first.event$peak_tail) & !is.na(first.event$Sex) & !is.na(first.event$QTc.adj) & first.event$Country != "France",]# & first.event$Country != "UK" & !is.na(first.event$traff_score)
t$Age_at_event_end[t$Age_at_event_end > age_cutoff] <- age_cutoff
t<-t[t$Age_at_event_end > t$Age_at_event_start,]
t$Event[t$Age_at_event_end == age_cutoff] <- 0
...
boot_auc <- function(data, indices) {
    ...
    roc_result <- survivalROC::survivalROC(
        ...
        method = "NNE",
        span = 0.10*length(valid_t1$valid_predicted_probs)^(-0.20)
    )
...
time_point <- 20 # years
boot_number <- 100 # bootstrap repetitions
...
roc_result2 <- survivalROC::survivalROC(Stime = valid_t2$Age_at_event_end, 
                                        ...
                                        predict.time = time_point, 
                                        method = "NNE",
                                        span = 0.10*length(valid_t2$valid_predicted_probs)^(-0.20))
...
# Fit the model 3
fit3 <- flexsurvspline(Surv(Age_at_event_start, Age_at_event_end, Event) ~ Sex + QTc.adj + peak_tail, data = t) # + traff_score + p_mean_w
...
roc_result3 <- survivalROC::survivalROC(Stime = valid_t3$Age_at_event_end, 
                                        ...
                                        predict.time = time_point, 
                                        method = "NNE",
                                        span = 0.10*length(valid_t3$valid_predicted_probs)^(-0.20))
...
# Prepare ROC curve data
roc_curve3 <- data.frame(Specificity = specificity3, Sensitivity = sensitivity3, Model = "Base Model + Peak Tail Current") #LQTS Penetrance + Trafficking + 

# Fit the model 1
fit4 <- flexsurvspline(Surv(Age_at_event_start, Age_at_event_end, Event) ~ Sex + QTc.adj + p_mean_w + peak_tail, data = t)

# Generate predictions and compute risk
predicted_probs4 <- 1 - predict(fit4, newdata = t, type = "survival", times = time_point)

# Convert to risk probabilities
valid_predicted_probs4 <- predicted_probs4$.pred_survival[!is.na(predicted_probs4$.pred_survival)]

# Filter the original dataset for valid cases
valid_t4 <- t[!is.na(predicted_probs4$.pred_survival),]
valid_t4$valid_predicted_probs <- valid_predicted_probs4
#valid_t1 <- valid_t1[valid_t1$Patient=="Family Member",]

# Compute ROC curve using valid data
roc_result4 <- survivalROC::survivalROC(Stime = valid_t4$Age_at_event_end, 
                                        status = valid_t4$Event, 
                                        marker = valid_t4$valid_predicted_probs, 
                                        predict.time = time_point, 
                                        method = "NNE",
                                        span = 0.10*length(valid_t4$valid_predicted_probs)^(-0.20))

# Extract sensitivity and specificity
sensitivity4 <- roc_result4$TP
specificity4 <- roc_result4$FP

set.seed(123)  # For reproducibility
boot_results4 <- boot(data = valid_t4, statistic = boot_auc, R = boot_number)
boot_coefs4 <- boot_results4$t

# Calculate mean and confidence intervals for each coefficient across rows
auc4 <- apply(boot_coefs4, 2, mean)
ci4 <- apply(boot_coefs4, 2, function(x) quantile(x, c(0.025, 0.975)))

# Prepare ROC curve data
roc_curve4 <- data.frame(Specificity = specificity4, Sensitivity = sensitivity4, Model = "Sex and QTc (Base Model) + Penetrance + Peak Tail")

# Combine ROC data from all models
all_roc_data <- rbind(roc_curve1, roc_curve2, roc_curve3, roc_curve4)
...
ggplot(all_roc_data, aes(x = Specificity, y = Sensitivity, color = Model)) +
  ...
  annotate("text", x = 0.6, y = 0.25, label = paste("AUC4:", round(auc4, 2), "[",round(ci4[1],2), "-", round(ci4[2],2), "]"), size = 4, color = "black")
...
```{r}
age_cutoff <- 40
t <- first.event[!is.na(first.event$peak_tail) & !is.na(first.event$p_mean_w) & !is.na(first.event$traff_score) & first.event$Country != "France",]
t$Age_at_event_end[t$Age_at_event_end > age_cutoff] <- age_cutoff
t$Event[t$Age_at_event_end == age_cutoff] <- 0
...
model <- Surv(Age_at_event_start, Age_at_event_end, Event) ~ 
                   Sex + peak_tail#QTc.adj + p_mean_w #++ peak_tail# traff_score 
...
```{r}
age_cutoff <- 40
t <- first.serious.event[!is.na(first.serious.event$Sex) & !is.na(first.serious.event$QTc.adj) & !is.na(first.serious.event$p_mean_w) & !is.na(first.serious.event$peak_tail) & !is.na(first.serious.event$traff_score) & first.serious.event$Country != "France",]# & first.serious.event$Country != "UK",]# & first.serious.event$Sex == "Male",]
t$Age_at_event_end[t$Age_at_event_end > age_cutoff] <- age_cutoff
t$SeriousCardiacEvent[t$Age_at_event_end == age_cutoff] <- 0

model <- Surv(Age_at_event_start, Age_at_event_end, SeriousCardiacEvent) ~ 
                   Sex + QTc.adj + p_mean_w + peak_tail #+ traff_score#+ pore #+ peak_tail+ revel_score + am_pathogenicity + traff_score + clinvar
...
age_cutoff <- 40
t <- first.serious.event[!is.na(first.serious.event$peak_tail) & !is.na(first.serious.event$traff_score) & !is.na(first.serious.event$Sex) & !is.na(first.serious.event$QTc.adj) & first.serious.event$Country != "France" & first.serious.event$Country != "UK",]
t$Age_at_event_end[t$Age_at_event_end > age_cutoff] <- age_cutoff
t<-t[t$Age_at_event_end > t$Age_at_event_start,]
t$SeriousCardiacEvent[t$Age_at_event_end == age_cutoff] <- 0
...
# Time point for AUC calculation, choose a time that makes sense for your study
time_point <- 20 # years
boot_number <- 100 # bootstrap repetitions, production 500-1000

# Fit the model 1
fit1 <- flexsurvspline(Surv(Age_at_event_start, Age_at_event_end, SeriousCardiacEvent) ~ Sex + QTc.adj, data = t)
...
# Fit the model 2 
fit2 <- flexsurvspline(Surv(Age_at_event_start, Age_at_event_end, SeriousCardiacEvent) ~ Sex + QTc.adj + p_mean_w, data = t)
...
# Fit the model 3
fit3 <- flexsurvspline(Surv(Age_at_event_start, Age_at_event_end, SeriousCardiacEvent) ~ Sex + QTc.adj + p_mean_w + peak_tail + traff_score, data = t)
...
t <- cohort.var[ !is.na(cohort.var$lqt2_dist) & !is.na(cohort.var$revel_score) &  !is.na(cohort.var$p_mean_w) & !is.na(cohort.var$Age_at_event_end) & !is.na(cohort.var$traff_score) & cohort.var$Country != "France" & !is.na(cohort.var$peak_tail),]# & cohort.var$Country != "UK",]#  & cohort.var$Sex=="Female" 
...
cox_model <- coxph(Surv(Age_at_event_start, Age_at_event_end, Event, type = "counting") ~
                    strata(Sex) + QTc.adj + traff_score + p_mean_w + revel_score + am_pathogenicity + clinvar + pore  , data = t, cluster = Unique_ID) # + p_mean_w + traff_score + peak_tail + clinvar + revel_score + am_pathogenicity+ peak_tail
...
Cross-validated, Ridge Regression Cox proportional Hazards
...
y <- Surv(t$Age_at_event_start, t$Age_at_event_end, t$Event, type = "counting")
strata <- as.integer(factor(t$Sex))  # Ensure this matches the number of rows in `t`
y2 <- stratifySurv(y, strata)
x <- model.matrix(~ QTc + traff_score + p_mean_w + am_pathogenicity + clinvar + pore - 1, data = t) #p_mean_w + + peak_tail + revel_scorepeak_tail + 
x_scaled <- scale(x)  # Scales each variable in the matrix

set.seed(123)  # for reproducibility
cv_fit <- cv.glmnet(x_scaled, y2, family = "cox", alpha = 1)
best_lambda <- cv_fit$lambda.1se
final_model <- glmnet(x_scaled, y2, family = "cox", lambda = best_lambda, alpha=1)
...
lasso_model <- glmnet(x_scaled, y2, family = "cox")
plot(lasso_model, xvar = "lambda", label = TRUE)
abline(v = log(cv_fit$lambda.1se), col = "red")
...
ci_results <- bootstrap_ci(final_model, t, x_scaled, n_boot=500)
...
plot_data$Variable <- factor(plot_data$Variable, levels = c("QTc.adj", setdiff(plot_data$Variable, "QTc.adj")))
...
model <- Surv(Age_at_event_start, Age_at_event_end, Event, type = "counting") ~ #
        strata(Sex) + QTc.adj #+ p_mean_w 

age_cutoff <- 40
t <- cohort.var[ !is.na(cohort.var$p_mean_w)  & cohort.var$Country != "France",]# & cohort.var$Country != "UK",] #!is.na(cohort.var$peak_tail) &  & !is.na(cohort.var$traff_score)
t$Age_at_event_end[t$Age_at_event_end > age_cutoff] <- age_cutoff
t<-t[t$Age_at_event_end > t$Age_at_event_start,]
t$Event[t$Age_at_event_end == age_cutoff] <- 0
...
roc_data <- pROC::roc(response = t$Event, predictor = predicted_risks, 
                      times = 20, cause = 1)
...
nom <- nomogram(cph_model, fun = exp, fun.at=c(0.05,0.25,0.5,1,2,3,4,5), funlabel="Hazard Ratio")#fun = plogis, fun.at = c(.25, .5, .75), funlabel = "Survival Probability"
...
```{r}
model <- Surv(Age_at_event_start, Age_at_event_end, Event, type = "counting") ~ #SeriousCardiac
         QTc.adj + peak_tail#p_mean_w # traff_score  #

age_cutoff <- 40
t <- first.serious.event[!is.na(first.serious.event$Age_at_event_end) & !is.na(first.serious.event$p_mean_w) & first.serious.event$Country != "France",]# & !is.na(first.serious.event$peak_tail) & first.serious.event$Country != "UK",]# &  !is.na(first.serious.event$traff_score) , ]
t <- cohort.var[!is.na(cohort.var$Age_at_event_end) & !is.na(cohort.var$p_mean_w) &  cohort.var$Country != "France",]#  & !is.na(cohort.var$peak_tail) & cohort.var$Country != "UK" & !is.na(cohort.var$traff_score), ]# 
t$Age_at_event_end[t$Age_at_event_end > age_cutoff] <- age_cutoff
t<-t[t$Age_at_event_end > t$Age_at_event_start,]
t$Event[t$Age_at_event_end == age_cutoff] <- 0
...
cph_model <- cph(model, data = t, x=TRUE, y=TRUE, surv=TRUE)
...
nom_male <- nomogram(cph_model_male,  fun = exp, fun.at=c(0.25,0.5,1,2,3,4,5), funlabel="Hazard Ratio") #fun = plogis, fun.lp.at = c( -2, -1, 0.0, 2), funlabel = "Survival Probability")
nom_female <- nomogram(cph_model_female,  fun = exp, fun.at=c(0.25,0.5,1,2,3,4,5), funlabel="Hazard Ratio") #fun = plogis, fun.lp.at = c( -2, -1, 0.0, 2), funlabel = "Survival Probability")
nom <- nomogram(cph_model,  fun = exp, fun.at=c(0.25,0.5,1,2,3,4,5), funlabel="Hazard Ratio")
  
plot(nom_male) # Nomogram for males
plot(nom_female) # Nomogram for females
plot(nom) # Nomogram for females
