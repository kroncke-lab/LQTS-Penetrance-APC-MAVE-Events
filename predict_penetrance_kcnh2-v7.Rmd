...
covariates <- c("revel_score", "lqt2_dist", "traff_score", "am_pathogenicity")
...
covariates <- c("revel_score", "lqt2_dist", "am_pathogenicity") 
...
model <- Surv(Age_at_event_start, Age_at_event_end, SeriousCardiacEvent) ~ 
                   Sex + QTc.adj + p_mean_w + traff_score + revel_score + clinvar + am_pathogenicity
...
model <- Surv(Age_at_event_start, Age_at_event_end, Event) ~ 
                   Sex + QTc.adj + p_mean_w + peak_tail #+ traff_score + revel_score + clinvar + am_pathogenicity
...
x <- model.matrix(~ strata(Sex) + QTc.adj + traff_score + peak_tail + p_mean_w + time_sex + revel_score + am_pathogenicity + clinvar, data = t) #
set.seed(123)  # for reproducibility
...
cox_model <- coxph(Surv(Age_at_event_start, Age_at_event_end, Event, type = "counting") ~ strata(Sex) + QTc.adj + traff_score + peak_tail + p_mean_w + revel_score + am_pathogenicity + clinvar, data = t)
...
nom <- nomogram(cph_model, fun = exp, fun.at=c(0.05,0.25,0.5,1,2,3,4,5), funlabel="Hazard Ratio")
